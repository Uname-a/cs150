...
\paragraph{7.2 one to one ontto and inverse functions}def $ F: X -> y$ is one to one if for all $x_1,x_2 \in X $ if $F(x_1) = F(x_2) $ then $ x_1 = x_2$ $ <=>\; if x_1 != x_2, \; then \; F(x_1) != F(x_2)$ \\ negation $ F : X -> Y $ is not one to one iff $ \exists x_1 , x_2 \in X $ such that $ F(x_1) = F(x_2) $ and $ x_1 != x_2 $  \\ to prove a function is one to one : \\ 1 Suppose $ x_1, x_2 \in X \; such \; that \; f(x_1) = f(x_2) $ \\ 2 Prove $ x_1 = x_2 $  \\ onto functions \\ def $F: X->Y $ is surjective or onto iff $ \forall y \in Y , \exists x \in X $ such that $ F(x) = y$ Negation: $ F X -> Y $ isnot onto iff $ \exists y \in Y $ such that $ \forall x \in X $  $ F(x) != y$ \\ to prove a function is onto \\ 1) Suppose y is any element of y \\ 20 show $\exists x \in X $ such that $ F(x) = y $ \\ Def $ F : X -> Y $ is a one to one correspondence or a bijection iff F is both one to one and onto  \\ Def: if $ F: X -> Y $ is a bijection then there is an invers function $F^(-1) : Y -> X  $ such that $ F^(-1) (y) = x <=> F(x) = y $ \\ 7.2 homework : 5,10,16,36,40
\paragraph{7.3 composition of functions}Def : let $ F: X -> Y $ and $ g: Y -> Z $ such that the range of f is contained in the domain of g  \\ then $gof: x->Z $ is defined as $ (gof) (x) = g( f(x)) $ for all $ x \in X $ homework 7.3 1,3,9,16,21
\end{document} 