...
int helpWin(Board * p,int num,int row,int col,int rowDir,int colDir,int player,int count){
    ...
    if (player == 1){
        if (p->spaces[col][row] == 1 && col + colDir >= 0&& col + colDir < p->width && row + rowDir >= 0&& row + rowDir < p->height){
            ...
            row = row + rowDir;
            count ++;
            helpWin(p,num,row,col,rowDir,colDir,player,count);
        }
    ...
    else if(p->spaces[col][row] == 2 && col + colDir >= 0&& col + colDir < p->width && row + rowDir >= 0&& row + rowDir < p->height){
        if (p->spaces[col][row] == 2){
            col = col + colDir;
            row = row + rowDir;
            count ++;
            helpWin(p,num,row,col,rowDir,colDir,player,count);
...
int checkWin(Board * p,int player,int num,int move ){
    ...
    /*check = helpWin(p,num,row,move,0,0,player,0);
    check = helpWin(p,num,row,move,0,0,player,0);
    check = helpWin(p,num,row,move,0,0,player,0);
    check = helpWin(p,num,row,move,1,-1,player,0);
    check = helpWin(p,num,row,move,1,-1,player,0);
