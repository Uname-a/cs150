    ...
    for (i = 0; i < count; ++i)
        {
            if (db[i]->YEAR < before )
                {
                printf("\"%s\" \"%s\" %d %d %s \"%s\" \"%s\"\n",db[i]->MOVIE_NAME, db[i]->DESCRIPTION,db[i]->YEAR,db[i]->LENGTH,db[i]->RATING,db[i]->CAST,db[i]->DIRECTOR);//formatted so that it is easy to read 

                }
        }
}
void moviesAfter(Movie **db,int count,int after)
{
    int i;
    for (i = 0; i < count; ++i)
        {
